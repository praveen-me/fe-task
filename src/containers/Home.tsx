...
export default function Home() {
  ...
  async function handleOperationOnJob(
    ...
    if (operation === "edit") {
      ...
      await helpers.getAllJobs(dispatch);
    }
  }
  ...
  return (
    <>
      <div className="flex flex-row justify-end my-8">
        <AppButton title="Create Job" handleSubmit={() => toggleModal()} />
      </div>

      <div className="flex flex-row justify-between mt-24 flex-wrap flex-1">
        {!isLoading ? (
          jobs.map((job) => (
            <JobCard
              key={job.id}
